...
module.exports = class extends (
  ...
  async run(oldMessage = Message, newMessage = Message) {
    if (!oldMessage.guild ||!newMessage.guild || oldMessage.author.bot|| newMessage.author.bot) return;
    ...
    if (
      ...
      embed.addField("❯ Modified Message", `${msg.substring(0, 1020)}` || "\u200b");
    }
    embed
      .addField("Link!!", `[Click here to see the message](${newMessage.url})`, true)
      .setTimestamp(oldMessage.editedAt || newMessage.editedAt || new Date())
      .setFooter("Edited");
  
  const channelLogs = oldMessage.guild.channels.cache.get(settings.logchannelID);
